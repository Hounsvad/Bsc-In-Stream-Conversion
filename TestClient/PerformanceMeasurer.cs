    ...
    public class PerformanceMeasurer
    {
        ...
        public static async Task DumpLog()
        {
            try
            {
                ...
                using (var fs = new StreamWriter(new FileStream(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + $"/UnitPerformance/{StartTime}ClientDump{++NumberOfLogFiles:000}.txt", FileMode.OpenOrCreate)))
                {
                    _lock.WaitOne();
